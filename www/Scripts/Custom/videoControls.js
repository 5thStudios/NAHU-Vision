...
$(function () {
    ...
    var btnBack = $('#btnBack');
    var lblPercent = $('#lblPercent');
    var secondsBack = 15;
    var recordedVideoStatus = false;
    ...
    player.on('timeupdate', function (data) {
        ...
        lblPercent.html((data.percent * 100).toFixed(1) + ' %');

        //
        if (recordedVideoStatus == false) {
            console.log(data.percent);
            if (data.percent >= .980) {
                console.log('***VIDEO COMPLETE***');
                recordedVideoStatus = true;

                //Does cookie exist?
                var FullLoginResponse = Cookies.get('FullLoginResponse');
                console.log(FullLoginResponse);
                ...
                if (FullLoginResponse === undefined) {
                    ...
                    var Type = "POST";
                    var Url = "/Services/ImisWebservice2017.asmx/SaveVideoCompletion";
                ...
                function CreateParameters() {
                    ...
                    var jsonFullResponse = JSON.parse(FullLoginResponse);
                    var videoId = $('#hfldPageId').val();

                    //Instantiate an array of parameters to pass to handler
                    var myData = {
                        imisId: jsonFullResponse.ImisUserId,
                        videoId: videoId
                    };
                ...
                function ServiceFailed(result) {
                    console.log('Save Video Completion call failed: ' + result.status + ' ' + result.statusText + ' ' + result.responseText);
                    Type = null;
                    varUrl = null;
                    Data = null;
                    ContentType = null;
                ...
                function ServiceSucceeded(result) {
                    console.log('Save Video Completion call succeeded.');
    ...
    btnBack.on('click', function () {
        //Obtain the current time and go back ~n number of seconds.
        var currentSeconds = 0;
        player.getCurrentTime().then(function (seconds) {
            currentSeconds = (seconds - secondsBack);
            if (currentSeconds < 0) currentSeconds = 0;
        }).then(function () {
            player.setCurrentTime(currentSeconds).catch(function (err) {
                console.log(err);
            });
